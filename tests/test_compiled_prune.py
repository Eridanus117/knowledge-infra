...
def test_retired_repo_dir_pruned(tmp_path: Path) -> None:
    ...
    _seed_repo_dir(compiled, "source-alpha", 3)  # active
    _seed_repo_dir(compiled, "source-alpha", 2)  # 退役 leaf 名孤儿
    rp = prune_retired_repos(compiled, {"source-alpha"}, apply=True)
    assert rp.retired == ["source-alpha"]
    assert rp.deleted is True
    assert not (compiled / "source-alpha").exists()
    assert (compiled / "source-alpha").exists()
