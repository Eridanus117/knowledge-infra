...
Collision keys are computed by applying locale-independent Unicode 17.0 full case folding after NFC normalization, not lowercase conversion or simple case folding. Display values remain NFC and are not replaced by folded keys. Two physical domain directories with the same folded C2 key are a domain collision. Two in-domain notes with the same folded full identity are an identity collision, including equal filename stems below different non-domain physical subdirectories.

### Bounded source walk

`SourceContext.git_root` must canonicalize to an existing directory with an exact `.git` entry that is a file or directory. `SourceSpec.root` is canonicalized again at discovery and may equal the Git root or be strictly below it; canonical containment is required. A lexical child that resolves outside through a source-root symlink is outside the Git root. On Windows, canonical local drive paths are supported; UNC, device, and other prefixes that cannot be rooted without ambient full-path traversal fail with `KBV2-SOURCE-GIT-ROOT`.

Discovery is iterative and bounded by the canonical source root. Ambient authority is used only to open the immutable Unix filesystem root or Windows drive root; the canonical Git root is opened from that anchor one segment at a time with final-component symlink following disabled, and its opened-handle metadata must identify a directory. That retained Git handle is the authority boundary. The source and every pending child directory are likewise opened one segment at a time relative to the retained Git or source handle with final-component symlink following disabled and directory metadata verified. Candidate files are opened by bare entry name relative to the current retained directory; Unix opens are nonblocking, and opened-handle metadata must identify a regular file before any bytes are read. The opened file's length is snapshotted, the read is capped at that length plus one byte, and any growth or truncation is a read failure. A candidate is parsed immediately and its file handle is dropped. The traversal stack stores only source-relative paths and domain lineage, so the number of live handles is bounded independently of directory depth and width. Stored paths are used only for no-follow resolution beneath the retained source capability and for diagnostics, ordering, and snapshot locators, never for an ambient open. Therefore a static or concurrent symlink, junction, rename, or special-file substitution cannot escape the capability or redirect a later ambient pathname open. Discovery never traverses a directory symlink. Before reading directory contents below an entry, it prunes `.git`, `.obsidian`, every dot-prefixed directory, `.venv`, `.legacy-index`, `node_modules`, `target`, and `dist`.
...
`SourceSnapshot.domains` is ordered by `DomainId` and then normalized source-relative POSIX (`/`) path string. `SourceSnapshot.notes` is ordered by domain and then the same normalized source-relative POSIX path string; native `PathBuf` component ordering is not the contract. Filesystem enumeration order, physical Git-root name, and registry row order do not affect the result. Discovery returns the complete snapshot or diagnostics, never a partial snapshot.

### Domain, identity, and discovery diagnostics

Every diagnostic below has `severity: Severity::Error`. Messages are exact and never append parser output, OS errors, private paths, source names, or offending text. Collision failures produce one diagnostic at each colliding structural path; for a two-path collision this is exactly two diagnostics ordered by folded logical key and then normalized source-relative POSIX (`/`) path string.
...
| `KBV2-SOURCE-GIT-ROOT` | Git root is missing, not a directory, unreadable, uses an unsupported capability-unsafe Windows prefix, or has no exact file/directory `.git` entry | attempted Git root | none | `Git root must be an existing directory containing .git` |
