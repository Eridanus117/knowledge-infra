...
`gnomon` core (identical `calls` schema across source-gamma tools, so ledgers can
...
`cli.run()` owns the capture loop itself (mirrors source-beta.cli.run_wrapped) rather
